...
import com.fangbian365.kuaidi.ui.adapter.CategoryAdapter;
import com.fangbian365.kuaidi.ui.adapter.MenuProductAdapter;
 ...
 *
 */
public class AddProductFragment extends Fragment implements IDataTransfer {
	...
	private List<HaveProductBean> mList = new ArrayList<HaveProductBean>();

	private ProductList<HaveProductBean> addList = new ProductList<HaveProductBean>();
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (bundle != null) {
			table = (Canyin_Shop_Diningtable) bundle
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		ViewGroup view = (ViewGroup) inflater.inflate(
				R.layout.layout_addproduct, null);

		categoryGroup = (ViewGroup) view.findViewById(R.id.category_linear);
		popLayerWidth = categoryGroup.findViewById(R.id.category_linear)
				.getLayoutParams().width;

//		hideCategoryImg = (ImageView) categoryGroup
...
//		});
	...
	public void onViewCreated(View view, Bundle savedInstanceState) {
		...
		getProductCategory();
		productAdapter = new MenuProductAdapter(getActivity());
		for (Canyin_Shop_Product product : obtainDishClass(0)) {
			HaveProductBean proBean = new HaveProductBean(product, table);
			proBean.setCnt(0.0f);
			mList.add(proBean);
		}
		...
		categoryList.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> adapter, View view, int position, long arg3) {
				...
				for (Canyin_Shop_Product products : productList) {
					HaveProductBean proBean = new HaveProductBean(products, table);
					proBean.setCnt(0.0f);
					mList.add(proBean);
				}

				for (HaveProductBean item : mList) {
					if (addList.contains(item)) {
						for (HaveProductBean addItem : addList) {
	...
	private void hidePopupMenu() {
		...
		TranslateAnimation animation = new TranslateAnimation(0, popLayerWidth,
				0, 0);
		animation.setDuration(400);
	...
	public void onDataTransfer(int type, Object... data) {
		if (type == Constans.TYPE_MENU_PRO_ADD_SUB) {
			HaveProductBean bean = (HaveProductBean) data[0];
			addList.add(bean, 0);
		}
